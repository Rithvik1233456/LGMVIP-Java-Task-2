...
{  
    ...
    TicTacToeGameinSwing() 
    {
        ...
        frame.getContentPane().setBackground(new Color(100, 4, 60));
        ...
        for (int i = 0; i < 9; i++) 
        {
            bton[i] = new JButton();
            bt_panel.add(bton[i]);
            bton[i].setFont(new Font("Arial", Font.BOLD, 120));
            bton[i].setFocusable(false);
            bton[i].addActionListener(this);
            bton[i].setBackground(new Color(255,229,204));
        }      
        ...
        for (int i = 0; i < 9; i++) 
        {
            if (e.getSource() == bton[i]) 
            {
                ...
                else 
                {
                    if (bton[i].getText() == "") 
                    {
                        bton[i].setForeground(new Color(0, 255, 0));
                        bton[i].setText("O");
                        pl1_chance = true;
